...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_currency_exchange_rate',"<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Exchange Rate Entry Saved successfully', 1);
			...
			fields : {
				...
				to_currency : {
					validators : {
						...
						callback: {
                            message: 'From Currency And To Currency is not be Same ',
                            callback: function (value, validator, $field) {
                            	return (value != $("#from_currency").val());
                            }
						}
					}
				},
		...
		fnBulkSave("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Exchange Rate</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/currency-exchange-rate', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	$(document).ready(function () {			
		populateOptionValues("from_currency","<?php echo $this->url('adminpanel/currencies', array('action'=>'getcurrency'));?>","Select From Currency");
		populateOptionValues("to_currency","<?php echo $this->url('adminpanel/currencies', array('action'=>'getcurrency'));?>","Select To Currency");
	});
</script>
