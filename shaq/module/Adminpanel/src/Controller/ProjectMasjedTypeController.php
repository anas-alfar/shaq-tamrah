...
class ProjectMasjedTypeController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`published`','`country`');
		if(!($this->memCached->hasItem('aula_masjedtype_data')) || !is_array($this->memCached->getItem('aula_masjedtype_data')))
		{	
			$sTable = 'view_project_masjed_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name FROM project_masjed_type_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_masjedtype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_masjedtype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_project_masjed_type WHERE 1 = 1";
			...
			$resultSet->initialize($resultData);        
			$rowset 			= $resultSet->toArray();
			$csvData .= "#ID,Published(Yes|No),Country,";
			foreach($activeLocalesArray as $locale)
			{
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name FROM project_masjed_type_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('project_masjed_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('project_masjed_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_project_masjed_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}														
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("project_masjed_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['project_masjed_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('project_masjed_type_locale',$existRecordID,'project_masjed_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_masjedtype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=project-masjed-type.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_masjedtype_data') && is_array($this->memCached->getItem('aula_masjedtype_data')))
			{
				$project_masjed_type = $this->memCached->getItem('aula_masjedtype_data');
				$rowset[0] = $project_masjed_type[$iID];
			}
			else
			{
				$projectTable = new TableGateway('project_masjed_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("project_masjed_type_id=".$iMasterID));
				$this->memCached->setItem('aula_masjedtype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'project_masjed_type';
		$tableNameLocale = 'project_masjed_type_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];	
					$detailData['project_masjed_type_id'] = $iMasterID;		
					$detailData['owner_organization_id'] = self::$Aula_OwnerOrgID;
					$detailData['owner_organization_user_id'] = self::$Aula_OwnerOrgUserID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("project_masjed_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getpostcategoryAction() 
	  {                
		$sql="select project_masjed_type_id as id,name as name from project_masjed_type_locale where locale_id = '".$this->global_locale_id."' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
