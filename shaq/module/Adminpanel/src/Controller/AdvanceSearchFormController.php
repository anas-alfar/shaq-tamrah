...
class AdvanceSearchFormController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_maritalstatus_data')) || !is_array($this->memCached->getItem('aula_maritalstatus_data')))
		{	
			$sTable = 'view_marital_status';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM marital_status_locale WHERE locale_id = '".$locale['id']."' AND marital_status_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_maritalstatus_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_maritalstatus_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_marital_status WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM marital_status_locale WHERE locale_id = '".$locale['id']."' AND marital_status_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('marital_status',$this->dbAdapter);
		$projectTableLocale = new TableGateway('marital_status_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_marital_status',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}		
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("marital_status_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['marital_status_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('marital_status_locale',$existRecordID,'marital_status_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_maritalstatus_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=marital_statuses.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_maritalstatus_data') && is_array($this->memCached->getItem('aula_maritalstatus_data')))
			{
				$marital_statuses = $this->memCached->getItem('aula_maritalstatus_data');
				$rowset[0] = $marital_statuses[$iID];
			}
			else
			{
				$projectTable = new TableGateway('marital_status', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("marital_status_id=".$iMasterID));
				$this->memCached->setItem('aula_maritalstatus_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'marital_status';
		$tableNameLocale = 'marital_status_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['marital_status_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("marital_status_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	 ...
	 public function getmaritalstatuspublishedAction() 
	  {                
		$sql="select id as id,name as name,published from view_marital_status where published='Yes' ";	
