...
class ProjectMasjedFurnitureTypeController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_masjedfurniture_data')) || !is_array($this->memCached->getItem('aula_masjedfurniture_data')))
		{	
			$sTable = 'view_project_masjed_furniture_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM project_masjed_furniture_type_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_furniture_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_masjedfurniture_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_masjedfurniture_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_project_masjed_furniture_type WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM project_masjed_furniture_type_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_furniture_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('project_masjed_furniture_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('project_masjed_furniture_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_project_masjed_furniture_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("project_masjed_furniture_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['project_masjed_furniture_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('project_masjed_furniture_type_locale',$existRecordID,'project_masjed_furniture_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_masjedfurniture_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=project-masjed-furniture-types.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_masjedfurniture_data') && is_array($this->memCached->getItem('aula_masjedfurniture_data')))
			{
				$project_masjed_furniture_typees = $this->memCached->getItem('aula_masjedfurniture_data');
				$rowset[0] = $project_masjed_furniture_typees[$iID];
			}
			else
			{
				$projectTable = new TableGateway('project_masjed_furniture_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("project_masjed_furniture_type_id=".$iMasterID));
				$this->memCached->setItem('aula_masjedfurniture_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'project_masjed_furniture_type';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'project_masjed_furniture_type';
		$tableNameLocale = 'project_masjed_furniture_type_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['project_masjed_furniture_type_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("project_masjed_furniture_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
