...
class MediaFilesTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_media_filetype_data')) || !is_array($this->memCached->getItem('aula_media_filetype_data')))
		{	
			$sTable = 'view_media_filetype';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM media_filetype_locale WHERE locale_id = '".$locale['id']."' AND media_filetype_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_media_filetype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_media_filetype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM media_filetype WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM media_filetype_locale WHERE locale_id = '".$locale['id']."' AND media_filetype_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('media_filetype',$this->dbAdapter);
		$projectTableLocale = new TableGateway('media_filetype_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_media_filetype',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("media_filetype_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['media_filetype_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('media_filetype_locale',$existRecordID,'media_filetype_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_media_filetype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=media_filetypes.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'media_filetype_id',$this->dbAdapter,$this->config);       
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_media_filetype_data') && is_array($this->memCached->getItem('aula_media_filetype_data')))
			{
				$media_filetype = $this->memCached->getItem('aula_media_filetype_data');
				$rowset[0] = $media_filetype[$iID];
			}
			else
			{
				$projectTable = new TableGateway('media_filetype', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("media_filetype_id=".$iMasterID));
				$this->memCached->setItem('aula_media_filetype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'media_filetype';
		$tableNameLocale = 'media_filetype_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['media_filetype_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("media_filetype_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getmediafiletypeAction() 
	  {                
		$sql="select id as id,name as name,published from view_media_filetype where published='Yes' ";   
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
