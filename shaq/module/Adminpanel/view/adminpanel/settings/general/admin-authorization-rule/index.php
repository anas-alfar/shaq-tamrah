...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#admin_authorization_role_id").val(), 'admin_authorization_rule', "admin_authorization_role_id", "<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Role is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Rule Entry Saved successfully', 1);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Rule</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/admin-authorization-rule', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
populateOptionValues("admin_authorization_role_id","<?php echo $this->url('adminpanel/admin-authorization-role', array('action'=>'getrole'));?>","Select Roles");
populateOptionValues("authorization_resource_id","<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'getresource'));?>","Select Resource");
</script>