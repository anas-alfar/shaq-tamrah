...
class PostCategoriesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		{	
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM post_category_locale WHERE locale_id = '".$locale['id']."' AND post_category_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_categories_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_categories_data');
		$iTotal = count($rowset);
	...
	public function getgriddetailslistAction()
	{
		...
		$type = $this->AdminfunctionsPlugin()->getSingleRecord2('post_category_id',$iID,'view_post_type',$this->dbAdapter);	
			$grid_list = '';
			$grid_list .= '<h5 class="gridDetailSectionHeading">Post Type:';
			$grid_list .= '<button class="btn addEventBtn" id="btnPostType" row-id="'.$iID.'">
								<i class="fa fa-plus"></i> &nbsp;Add Post Type
							</button></h5>';
			$grid_list .= '<table cellpadding="5" cellspacing="0" border="0" class="table table-hover table-condensed gridDetailTable" id="post_type_tbl">';
			if(count($type) > 0)
			{	
	...
	{	
		$projectTable = new TableGateway('post_category',$this->dbAdapter);	 
        if ($this->request->isPost()) {		
			...
			$this->memCached->setItem('aula_categories_data','');
			$result['DBStatus'] = 'OK';
		}
		else
        {
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_post_category WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM post_category_locale WHERE locale_id = '".$locale['id']."' AND post_category_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('post_category',$this->dbAdapter);
		$projectTableLocale = new TableGateway('post_category_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
						...
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_post_category',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
								if($existRecordID > 0)
								{
									continue;
								}								
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("post_category_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['post_category_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('post_category_locale',$existRecordID,'post_category_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=post-category.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'post_category_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_categories_data') && is_array($this->memCached->getItem('aula_categories_data')))
			{
				$organization = $this->memCached->getItem('aula_categories_data');
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("post_category_id=".$iMasterID));
				$this->memCached->setItem('aula_categories_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'post_category';
		$tableNameLocale = 'post_category_locale';
        if ($this->request->isPost()) {
			...
			{
				$sql="select sequence from post_category order by sequence DESC LIMIT 1 ";
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['post_category_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['name'] = $aData['name_'.$locale['id']];
					$detailData['description'] = $aData['description_'.$locale['id']];
					$rowset = $projectTableLocale->select(array("post_category_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getpostcategoryAction() 
	  {                
		$sql="select post_category_id as id,name as name from post_category_locale where locale_id = '".$this->global_locale_id."' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
