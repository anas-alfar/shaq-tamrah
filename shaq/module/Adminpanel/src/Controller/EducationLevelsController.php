...
class EducationLevelsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_educationlevel_data')) || !is_array($this->memCached->getItem('aula_educationlevel_data')))
		{	
			$sTable = 'view_education_level';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM education_level_locale WHERE locale_id = '".$locale['id']."' AND education_level_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_educationlevel_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_educationlevel_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_education_level WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM education_level_locale WHERE locale_id = '".$locale['id']."' AND education_level_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('education_level',$this->dbAdapter);
		$projectTableLocale = new TableGateway('education_level_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("education_level_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['education_level_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('education_level_locale',$existRecordID,'education_level_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_educationlevel_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=education_leveles.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_educationlevel_data') && is_array($this->memCached->getItem('aula_educationlevel_data')))
			{
				$education_level = $this->memCached->getItem('aula_educationlevel_data');
				$rowset[0] = $education_level[$iID];
			}
			else
			{
				$projectTable = new TableGateway('education_level', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("education_level_id=".$iMasterID));
				$this->memCached->setItem('aula_educationlevel_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'education_level';
		$tableNameLocale = 'education_level_locale';
        if ($this->request->isPost()) {
