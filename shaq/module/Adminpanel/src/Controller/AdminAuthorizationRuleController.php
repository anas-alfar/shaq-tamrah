...
class AdminAuthorizationRuleController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		$aColumns = array( '`id`','`roles`','`resources`','`permission`','`organization`');
		if(!($this->memCached->hasItem('aula_adminauthorizationrule_data')) || !is_array($this->memCached->getItem('aula_adminauthorizationrule_data')))
		{	
			$sTable = 'view_admin_authorization_rule';
			...
			$this->memCached->setItem('aula_adminauthorizationrule_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_adminauthorizationrule_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';
			$sql = "SELECT * FROM view_admin_authorization_rule WHERE 1 = 1";	
			...
			$csvData .= "#ID,Roles,Resources,Permission(allow|deny),Organization";
			$csvData .= "\n";
			foreach($rowset as $row)
			{
				$csvData .= $row['id'].",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['roles']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['resources']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['permission']).",";
    ...
    {			
        $projectTable = new TableGateway('admin_authorization_rule',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "")
							{
								...
								$getRoleID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('admin_authorization_role',$this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]),'name','id',$this->dbAdapter,$this->config['global_locale_id']);
								
								$saveDataArray['admin_authorization_role_id'] 	= $getRoleID;
								
								$getResourceID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('admin_authorization_resource',$this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]),'name','id',$this->dbAdapter,$this->config['global_locale_id']);
								$saveDataArray['authorization_resource_id']		= $getResourceID;
								
								$saveDataArray['permission']					= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('admin_authorization_rule',$saveDataArray['admin_authorization_role_id'],'admin_authorization_role_id',$this->dbAdapter,$data[0]); 						
								if($existRecordID > 0)
									{
									continue;
									}															
						...
						$this->memCached->setItem('aula_adminauthorizationrule_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	{
		$csvData = '';		
		$csvData .= "Roles,Resources,Permission(allow|deny),Organization";
		$csvData .= "\n";
		header('Content-Type: text/csv; charset=utf-8');
		header("Content-Disposition: attachment; filename=admina-uthorization-rule.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_adminauthorizationrule_data') && is_array($this->memCached->getItem('aula_adminauthorizationrule_data')))
			{
				$admin_authorization_rule = $this->memCached->getItem('aula_adminauthorizationrule_data');
				$rowset[0] = $admin_authorization_rule[$iID];
			}
			else
			{
				$projectTable = new TableGateway('admin_authorization_rule', $this->dbAdapter);
    ...
    {        
		$tableName = 'admin_authorization_rule';
        if ($this->request->isPost()) {
