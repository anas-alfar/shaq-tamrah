...
class PaymentMethodController extends AbstractActionController
{
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`id`','`name`','`country_name`','`published`');
		if(!($this->memCached->hasItem('aula_paymentmethod_data')) || !is_array($this->memCached->getItem('aula_paymentmethod_data')))
		{	
			$sTable = 'view_payment_method';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name FROM payment_method_locale WHERE locale_id = '".$locale['id']."' AND payment_method_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_paymentmethod_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_paymentmethod_data');
		$iTotal = count($rowset);
	...
	public function getgriddetailslistAction()
	{
		...
		$configuration = $this->AdminfunctionsPlugin()->getSingleRecord2('payment_method_id',$iID,'view_payment_method_configuration',$this->dbAdapter);	
		
			$grid_list = '';
			$grid_list .= '<h5 class="gridDetailSectionHeading">Method Configuration:</h5>';
			...
			{	
				foreach($configuration as $configurationdata)
				{
					$grid_list .= '<tr >
										<td>'.$configurationdata['config_key'].'</td>
									</tr>';
				}
			}
			else
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_payment_method WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name FROM payment_method_locale WHERE locale_id = '".$locale['id']."' AND payment_method_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('payment_method',$this->dbAdapter);
		$projectTableLocale = new TableGateway('payment_method_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_payment_method',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("payment_method_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['payment_method_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('payment_method_locale',$existRecordID,'payment_method_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_paymentmethod_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=payment_method.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_paymentmethod_data') && is_array($this->memCached->getItem('aula_paymentmethod_data')))
			{
				$payment_methodes = $this->memCached->getItem('aula_paymentmethod_data');
				$rowset[0] = $payment_methodes[$iID];
			}
			else
			{
				$projectTable = new TableGateway('payment_method', $this->dbAdapter);
    ...
    {
				if ($this->request->getPost("pAction") == "POPUP") {
				
                	$iMasterID = $this->request->getPost("KEY_ID");
					$iMasterNAME = $this->request->getPost("NAME");
					
					$aColumns = array( '`id`','`config_key`','`config_value`','`locale`','`environment`','`payment_method`','`country_name`');
					$sTable = 'view_payment_method_configuration';
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("payment_method_id=".$iMasterID));
				$this->memCached->setItem('aula_paymentmethod_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'payment_method';
		$tableNameLocale = 'payment_method_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['payment_method_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("payment_method_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getpaymentAction() 
	  {                
		...
		$sql="select id as id,name as name,published from view_payment_method where published='Yes' ";	
