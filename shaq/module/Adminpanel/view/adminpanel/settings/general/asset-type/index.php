...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/asset-type', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		$('#tabs').tabs();
			...
			$('#tblMasterList tbody').on('click', 'td.details-control', function () {
				...
				else if(tr.hasClass('loaded'))
				{
					...
					$.ajax({
					  type: "POST",
					  url: "<?php echo $this->url('adminpanel/asset-type', array('action'=>'getgriddetailslist'));?>",
					  data:{'KEY_ID':KEY_ID},
					  dataType: "json",
					  success: function(data){
							hideShowLoader(false);
			...
			fields : {
				...
				asset_storage_type_id: {
                    validators: {
                       callback: {
                            message: 'Please select storage types',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
				...
				asset_unit_type_id: {
                    validators: {
                       callback: {
                            message: 'Please select unit types',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
		...
		fnBulkSave("<?php echo $this->url('adminpanel/asset-type', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/asset-type', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/asset-type', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/asset-type', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/asset-type', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/asset-type', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Asset Type</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/asset-type', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	...
	$(document).ready(function () {			
		...
		populateOptionValues("asset_storage_type_id","<?php echo $this->url('adminpanel/asset-storage-types', array('action'=>'getassetstoragetype'));?>","Select Storage Types");
		populateOptionValues("asset_unit_type_id","<?php echo $this->url('adminpanel/asset-unit-types', array('action'=>'getassetunit'));?>","Select Unit Types");
populateOptionValues("asset_condition_id","<?php echo $this->url('adminpanel/asset-conditions', array('action'=>'getcondition'));?>","Select Condition");
});
</script>