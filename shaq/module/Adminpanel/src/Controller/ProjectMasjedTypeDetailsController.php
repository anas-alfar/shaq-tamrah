...
class ProjectMasjedTypeDetailsController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`default_value`','`published`');
		if(!($this->memCached->hasItem('aula_masjedtypedetails_data')) || !is_array($this->memCached->getItem('aula_masjedtypedetails_data')))
		{	
			$sTable = 'view_project_masjed_type_details';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,default_value FROM project_masjed_type_details_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_type_details_id = '".$aRow['id']."' ";
					...
					$aRow['default_value_'.$locale['id']] = $rowset_locale[0]['default_value'];
				}
				$rowsetCache[$aRow['id']] = $aRow;
			}
			$this->memCached->setItem('aula_masjedtypedetails_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_masjedtypedetails_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_project_masjed_type_details WHERE 1 = 1";
			...
			foreach($activeLocalesArray as $locale)
			{
				...
				$csvData .= "Default Value(".$locale['name']."),";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,default_value FROM project_masjed_type_details_locale WHERE locale_id = '".$locale['id']."' AND project_masjed_type_details_id = '".$row['id']."' ";
						...
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['default_value']).",";
					}
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('project_masjed_type_details',$this->dbAdapter);
		$projectTableLocale = new TableGateway('project_masjed_type_details_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "" && $data[5] != "" )
							{
								...
								$detailData['default_value'] = $data[$column_index++];
								
								
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_project_masjed_type_details',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
									if($existRecordID > 0)
									{
										continue;
									}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("project_masjed_type_details_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['project_masjed_type_details_id'] = $existRecordID;
									$detailData['locale_id'] = $this->config['global_locale_id'];	
									$detailData['project_id'] = 1;						
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('project_masjed_type_details_locale',$existRecordID,'project_masjed_type_details_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_masjedtypedetails_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=project-masjed-type-details.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'project_masjed_type_details_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_masjedtypedetails_data') && is_array($this->memCached->getItem('aula_masjedtypedetails_data')))
			{
				$project_masjed_type_detailss = $this->memCached->getItem('aula_masjedtypedetails_data');
				$rowset[0] = $project_masjed_type_detailss[$iID];
			}
			else
			{
				$projectTable = new TableGateway('project_masjed_type_details', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("project_masjed_type_details_id=".$iMasterID));
				$this->memCached->setItem('aula_masjedtypedetails_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'project_masjed_type_details';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'project_masjed_type_details';
		$tableNameLocale = 'project_masjed_type_details_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['default_value'] 		= $aData['default_value_'.$locale['id']];
					$detailData['project_id'] 		= 1;
					$detailData['locale_id'] 		= $locale['id'];
					$detailData['project_masjed_type_details_id'] 	= $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("project_masjed_type_details_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
