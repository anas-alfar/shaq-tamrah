...
class DisabledTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_disabledtype_data')) || !is_array($this->memCached->getItem('aula_disabledtype_data')))
		{	
			$sTable = 'view_beneficiary_profile_disabled_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM beneficiary_profile_disabled_type_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_disabled_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_disabledtype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_disabledtype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_profile_disabled_type WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM beneficiary_profile_disabled_type_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_disabled_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('beneficiary_profile_disabled_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('beneficiary_profile_disabled_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_profile_disabled_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_profile_disabled_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_profile_disabled_type_locale',$existRecordID,'beneficiary_profile_disabled_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_disabledtype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_disabledtype_data') && is_array($this->memCached->getItem('aula_disabledtype_data')))
			{
				$beneficiary_profile_disabled_types = $this->memCached->getItem('aula_disabledtype_data');
				$rowset[0] = $beneficiary_profile_disabled_types[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_profile_disabled_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_profile_disabled_type_id=".$iMasterID));
				$this->memCached->setItem('aula_disabledtype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'beneficiary_profile_disabled_type';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'beneficiary_profile_disabled_type';
		$tableNameLocale = 'beneficiary_profile_disabled_type_locale';
        if ($this->request->isPost()) {
