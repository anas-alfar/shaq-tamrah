...
<script type="text/javascript">
		...
		$("#btnAssetType").click(function(){
			...
			$("#frmFormType").val();
			$('#frmFormType').bootstrapValidator("resetForm",true);
			$("#frmFormType").find("textarea").val("");
		});
		...
		function savefrmFormTypeData()  {
			...
			var objFormData =
			{
				name    			: $("#frmFormType").find("#name_<?php echo $this->global_locale_id; ?>").val(),
				country_id			: $("#frmFormType").find("#country_id").val()

			};
			var isDuplicate = fn_validate_duplicate_multiple('view_asset_type',"<?php echo $this->url('adminpanel/asset-type', array('action'=>'validateduplicate'));?>",0,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found.Asset Type for Selected Country is Already exists !', 0);
				return false;
			}
			var $form = $('#frmFormType');
			var objMasterData = $form.serializeObject();
			$.extend(objMasterData, { MASTER_KEY_ID: 0});
		...
		$('#tabs2').tabs();
			...
			$('#tblMasterList tbody').on('click', 'td.details-control', function () {
				...
				else if(tr.hasClass('loaded'))
				{
					...
					$.ajax({
					  type: "POST",
					  url: "<?php echo $this->url('adminpanel/asset', array('action'=>'getgriddetailslist'));?>",
					  data:{'KEY_ID':KEY_ID},
					  dataType: "json",
					  success: function(data){
							hideShowLoader(false);
		...
		$('#frmFormType').bootstrapValidator({
			...
			fields : {
				...
				asset_storage_type_id: {
                    validators: {
                       callback: {
                            message: 'Please select asset storage type',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
										...
										<section class="col-md-6">
											...
											<div class="form-group">
												<label class="control-label">Country<span>*</span></label>
													<select class="select2" id="country2_id" name="country_id" type="select">														
...
<script language="javascript">
	$(document).ready(function () {			
		...
		populateOptionValues("country2_id","<?php echo $this->url('adminpanel/countries', array('action'=>'getcountry'));?>","Select Country");
