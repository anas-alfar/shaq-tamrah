...
<script type="text/javascript">
		...
		function fetch_grid_data_draggable()
		{
			$("#menu-item-output").val('');
			hideShowLoader(true);
			var objFormData ={
				menu_id : parentId
			};
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/menu', array('action'=>'getmenulist'));?>",
			  data: objFormData,
			  dataType: "json",
			  success: function(data){
			  		hideShowLoader(false);
					var updateOutput = function(e) {
						...
						if (window.JSON) {
							$("#menu-item-output").val(window.JSON.stringify(list.nestable('serialize')));
						} else {
							mySmallAlert('Error...!', 'JSON browser support required for this demo.', 0);
							output.val('');
						}
					};
					$('#menuItemListId').html(data.draggable_menu_list).nestable().unbind('change').on('change', updateOutput);					
			  }
			});
		...
		function saveorderData()  {
			var menuorder=$("#menu-item-output").val();
			if(menuorder=='')
			{
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/menu', array('action'=>'savemenuorder'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					$("#menu-item-output").val('');
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Menu Item Order Saved successfully', 1);
					iActiveID = objMyPost.DATA.MY_ID;
