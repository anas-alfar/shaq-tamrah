...
class BeneficiaryRelationsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		{	
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name FROM beneficiary_relation_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_relation_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_beneficiaryrelation_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_beneficiaryrelation_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_relation WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name FROM beneficiary_relation_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_relation_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('beneficiary_relation',$this->dbAdapter);
		$projectTableLocale = new TableGateway('beneficiary_relation_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_relation_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_relation_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_relation_locale',$existRecordID,'beneficiary_relation_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_beneficiaryrelation_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=beneficiary-relations.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_beneficiaryrelation_data') && is_array($this->memCached->getItem('aula_beneficiaryrelation_data')))
			{
				$beneficiary_relationes = $this->memCached->getItem('aula_beneficiaryrelation_data');
				$rowset[0] = $beneficiary_relationes[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_relation', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_relation_id=".$iMasterID));
				$this->memCached->setItem('aula_beneficiaryrelation_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'beneficiary_relation';
        if ($this->request->isPost()) {
			...
			{
				$updateData = array();
				$updateData['allow_recurrence'] = $this->setCheckboxValue($aData,'allow_recurrence'.$iMasterID,'Yes','No');
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'beneficiary_relation';
		$tableNameLocale = 'beneficiary_relation_locale';
        if ($this->request->isPost()) {
			...
			$aData['allow_recurrence'] = $this->setCheckboxValue($aData,'allow_recurrence','Yes','No');
