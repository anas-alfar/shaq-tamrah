...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/death-reasons', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_death_reason',"<?php echo $this->url('adminpanel/death-reasons', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Death Reasons for Selected Country Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Death Reason Entry Saved successfully', 1);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/death-reasons', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Death Reason</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/death-reasons', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
