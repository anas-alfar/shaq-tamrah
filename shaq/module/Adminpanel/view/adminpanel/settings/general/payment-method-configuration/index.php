...
<script type="text/javascript">
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_payment_method_configuration',"<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Payment Method Configuration is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Payment Method Configuration Entry Saved successfully', 1);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Payment Method Configuration</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	...
	$(document).ready(function () {			
		...
		populateOptionValues("locale_id","<?php echo $this->url('adminpanel/locales', array('action'=>'getlocale'));?>","Select Locale");
		populateOptionValues("payment_method_id","<?php echo $this->url('adminpanel/payment-method', array('action'=>'getpayment'));?>","Select Payment Method");
	});
