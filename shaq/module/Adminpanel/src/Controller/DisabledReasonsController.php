...
class DisabledReasonsController extends AbstractActionController
{
						...
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								{
									$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_beneficiary_profile_disabled_reason',$detailData['name'],'name',$this->dbAdapter);	
									if($existRecordID > 0)
									{
										continue;
									}
	...
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'beneficiary_profile_disabled_reason_id',$this->dbAdapter,$this->config);           
        }
		else {
        ...
        if ($this->request->isPost()) {
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$projectTableLocale->update($detailData,array("beneficiary_profile_disabled_reason_id=".$iMasterID,"locale_id=".$locale['id']));
				}									
				$result['DBStatus'] = 'OK';
			}
			$this->memCached->setItem('aula_disabledreason_data','');
