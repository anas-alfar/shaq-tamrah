...
class OrganizationFlagsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_organizationflag_data')) || !is_array($this->memCached->getItem('aula_organizationflag_data')))
		{	
			$sTable = 'view_organization_flag';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT flag_name FROM organization_flag_locale WHERE locale_id = '".$locale['id']."' AND organization_flag_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_organizationflag_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_organizationflag_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_organization_flag WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT flag_name FROM organization_flag_locale WHERE locale_id = '".$locale['id']."' AND organization_flag_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('organization_flag',$this->dbAdapter);
		$projectTableLocale = new TableGateway('organization_flag_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("organization_flag_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['organization_flag_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('organization_flag_locale',$existRecordID,'organization_flag_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_organizationflag_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_organizationflag_data') && is_array($this->memCached->getItem('aula_organizationflag_data')))
			{
				$organization_flags = $this->memCached->getItem('aula_organizationflag_data');
				$rowset[0] = $organization_flags[$iID];
			}
			else
			{
				$projectTable = new TableGateway('organization_flag', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("organization_flag_id=".$iMasterID));
				$this->memCached->setItem('aula_organizationflag_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'organization_flag';
		$tableNameLocale = 'organization_flag_locale';
        if ($this->request->isPost()) {
