...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/message-templates', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#from_name_<?php echo $this->global_locale_id; ?>").val(), 'message_template_locale', "from_name", "<?php echo $this->url('adminpanel/message-templates', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/message-templates', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Message Template Entry Saved successfully', 1);
		...
		$('#tabs').tabs();
                ...
                "aoColumns": [
					...
					{ "bSearchable": true, "bSortable": true,
                        "mRender" : function (data, type, full) {							
							return grid_switch(full[0],'published',full[10],'Yes');							
						}
					},
			...
			fields : {
				...
				message_type: {
                    validators: {
                       callback: {
                            message: 'Please select message type2',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
				...
				to_mobile_number : {
					validators : {
						notEmpty : {
							message : 'Please enter from to mobile number'						
						},
						digits : {
				...
				to_email : {
					validators : {
						notEmpty : {
							message : 'Please enter from to email'						
						},
						emailAddress: {
		...
		fnBulkSave("<?php echo $this->url('adminpanel/message-templates', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/message-templates', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/message-templates', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/message-templates', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/message-templates', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/message-templates', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Message Template</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/message-templates', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	...
	$(document).ready(function () {			
		populateOptionValues("message_type_id","<?php echo $this->url('adminpanel/message-types', array('action'=>'getmessage'));?>","Select Message Type");
	});
