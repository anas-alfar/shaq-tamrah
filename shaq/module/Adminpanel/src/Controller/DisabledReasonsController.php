...
class DisabledReasonsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_disabledreason_data')) || !is_array($this->memCached->getItem('aula_disabledreason_data')))
		{	
			$sTable = 'view_beneficiary_profile_disabled_reason';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM beneficiary_profile_disabled_reason_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_disabled_reason_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_disabledreason_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_disabledreason_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_profile_disabled_reason WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM beneficiary_profile_disabled_reason_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_disabled_reason_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('beneficiary_profile_disabled_reason',$this->dbAdapter);
		$projectTableLocale = new TableGateway('beneficiary_profile_disabled_reason_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_beneficiary_profile_disabled_reason',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_profile_disabled_reason_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_profile_disabled_reason_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_profile_disabled_reason_locale',$existRecordID,'beneficiary_profile_disabled_reason_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_disabledreason_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=disabled-reasones.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_disabledreason_data') && is_array($this->memCached->getItem('aula_disabledreason_data')))
			{
				$beneficiary_profile_disabled_reasones = $this->memCached->getItem('aula_disabledreason_data');
				$rowset[0] = $beneficiary_profile_disabled_reasones[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_profile_disabled_reason', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_profile_disabled_reason_id=".$iMasterID));
				$this->memCached->setItem('aula_disabledreason_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'beneficiary_profile_disabled_reason';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'beneficiary_profile_disabled_reason';
		$tableNameLocale = 'beneficiary_profile_disabled_reason_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['beneficiary_profile_disabled_reason_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("beneficiary_profile_disabled_reason_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getdisableresonAction() 
	  {                
		$sql="select id as id,name as name,published from view_beneficiary_profile_disabled_reason where published='Yes'";      
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
