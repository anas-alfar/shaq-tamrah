...
class ProjectCategoriesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		{	
			...
			$this->memCached->setItem('aula_projectcategoriesr_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_projectcategoriesr_data');
		$iTotal = count($rowset);
	...
	{	
		$projectTable = new TableGateway('project_category',$this->dbAdapter);	 
        if ($this->request->isPost()) {		
			...
			$this->memCached->setItem('aula_projectcategoriesr_data','');
			$result['DBStatus'] = 'OK';
		}
		else
        {
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';
			$sql = "SELECT * FROM project_category WHERE 1 = 1";	
						...
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('project_category',$saveDataArray['name'],'name',$this->dbAdapter,$data[0]); 						
								if($existRecordID > 0)
								{
									continue;
								}	
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_projectcategoriesr_data') && is_array($this->memCached->getItem('aula_projectcategoriesr_data')))
			{
				$project_categorys = $this->memCached->getItem('aula_projectcategoriesr_data');
				$rowset[0] = $project_categorys[$iID];
			}
			else
			{
				$projectTable = new TableGateway('project_category', $this->dbAdapter);
    ...
    {        
        ...
        if ($this->request->isPost()) {
			...
			{	
				...
				$sql="select sequence from project_category order by sequence DESC LIMIT 1 ";
	...
	public function getprojectcategoryAction() 
	  {                
		$sql="select id as id,name as name from project_category ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
