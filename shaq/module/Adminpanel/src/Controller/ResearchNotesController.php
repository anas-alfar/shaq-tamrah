...
class ResearchNotesController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`researcher_name`','`researcher_recommendations`','`committee_member_name`','`beneficiary`','`support_type`','`expected_support_period`');
		if(!($this->memCached->hasItem('aula_research_notes_data')) || !is_array($this->memCached->getItem('aula_research_notes_data')))
		{	
			$sTable = 'view_beneficiary_profile_research_notes';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT small_business_idea_description,researcher_recommendations,researcher_name,notes,committee_recommendations,committee_member_name,committee_manager_name FROM beneficiary_profile_research_notes_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_research_notes_id = '".$aRow['id']."' ";
					...
					$aRow['small_business_idea_description_'.$locale['id']] = $rowset_locale[0]['small_business_idea_description'];
					$aRow['researcher_recommendations_'.$locale['id']] = $rowset_locale[0]['researcher_recommendations'];
					$aRow['researcher_name_'.$locale['id']] = $rowset_locale[0]['researcher_name'];
					$aRow['notes_'.$locale['id']] = $rowset_locale[0]['notes'];
					$aRow['committee_recommendations_'.$locale['id']] = $rowset_locale[0]['committee_recommendations'];
					$aRow['committee_member_name_'.$locale['id']] = $rowset_locale[0]['committee_member_name'];
					$aRow['committee_manager_name_'.$locale['id']] = $rowset_locale[0]['committee_manager_name'];
				}
				$rowsetCache[$aRow['id']] = $aRow;
			}
			$this->memCached->setItem('aula_research_notes_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_research_notes_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_profile_research_notes WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM beneficiary_profile_research_notes_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_research_notes_id = '".$row['id']."' ";
    ...
    {			
		...
		$projectTableLocale = new TableGateway('beneficiary_profile_research_notes_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_beneficiary_profile_research_notes',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}		
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_profile_research_notes_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_profile_research_notes_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_profile_research_notes_locale',$existRecordID,'beneficiary_profile_research_notes_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_research_notes_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=beneficiary_profile_research_noteses.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'beneficiary_profile_research_notes_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_research_notes_data') && is_array($this->memCached->getItem('aula_research_notes_data')))
			{
				$beneficiary_profile_research_noteses = $this->memCached->getItem('aula_research_notes_data');
				$rowset[0] = $beneficiary_profile_research_noteses[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_profile_research_notes', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_profile_research_notes_id=".$iMasterID));
				$this->memCached->setItem('aula_research_notes_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
        ...
        if ($this->request->isPost()) {
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("beneficiary_profile_research_notes_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
