...
class HomeConstructionTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_homeconstructiontype_data')) || !is_array($this->memCached->getItem('aula_homeconstructiontype_data')))
		{	
			$sTable = 'view_beneficiary_profile_home_construction_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM beneficiary_profile_home_construction_type_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_home_construction_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_homeconstructiontype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_homeconstructiontype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_profile_home_construction_type WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM beneficiary_profile_home_construction_type_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_home_construction_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('beneficiary_profile_home_construction_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('beneficiary_profile_home_construction_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_beneficiary_profile_home_construction_type',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_profile_home_construction_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_profile_home_construction_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_profile_home_construction_type_locale',$existRecordID,'beneficiary_profile_home_construction_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_homeconstructiontype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=home-construction-types.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'beneficiary_profile_home_construction_type_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_homeconstructiontype_data') && is_array($this->memCached->getItem('aula_homeconstructiontype_data')))
			{
				$beneficiary_profile_home_construction_typees = $this->memCached->getItem('aula_homeconstructiontype_data');
				$rowset[0] = $beneficiary_profile_home_construction_typees[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_profile_home_construction_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_profile_home_construction_type_id=".$iMasterID));
				$this->memCached->setItem('aula_homeconstructiontype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'beneficiary_profile_home_construction_type';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'beneficiary_profile_home_construction_type';
		$tableNameLocale = 'beneficiary_profile_home_construction_type_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['beneficiary_profile_home_construction_type_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("beneficiary_profile_home_construction_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function gethomeconstructiontypeAction() 
	  {                
		$sql="select id as id,name as name,published from view_beneficiary_profile_home_construction_type where published='Yes'";      
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
