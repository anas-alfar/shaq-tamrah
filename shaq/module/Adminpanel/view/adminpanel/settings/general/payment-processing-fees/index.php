...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			*/
			...
			var isDuplicate = fn_validate_duplicate_multiple('payment_processing_fees',"<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Payment Method For Selected Country is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Payment Processing Fees Entry Saved successfully', 1);
				 ...
				 currency : {
						validators : {
							notEmpty : {
								...
								callback: {
									   message: 'Maximum three digit allowed ',
									   callback: function (value, validator, $field) {
		...
		fnBulkSave("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Payment Processing Fees</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/payment-processing-fees', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
