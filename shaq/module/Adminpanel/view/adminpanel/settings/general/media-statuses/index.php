...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/media-statuses', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#name_<?php echo $this->global_locale_id; ?>").val(), 'media_status_locale', "name", "<?php echo $this->url('adminpanel/media-statuses', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Media Status Entry Saved successfully', 1);
		...
		fnExport("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/media-statuses', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Media Status</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/media-statuses', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
