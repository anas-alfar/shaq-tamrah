...
class AssetUnitTypesController extends AbstractActionController
{
						...
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_asset_unit_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
        ...
        if ($this->request->isPost()) {
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("asset_unit_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getassetunitAction() 
	  {                
		$sql="select id as id,name as name,published from view_asset_unit_type where published='Yes' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
