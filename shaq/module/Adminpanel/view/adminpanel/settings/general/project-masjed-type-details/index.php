...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#name_<?php echo $this->global_locale_id; ?>").val(), 'project_masjed_type_details_locale', "name", "<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Project Masjed Type Detail Entry Saved successfully', 1);
			...
			fields : {
				<?php
					foreach($this->activeLocalesArray as $locale)
					{
						...
						default_value_<?php echo $locale['id']?> : {
							validators : {
								notEmpty : {
		...
		fnBulkSave("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Project Masjed Type Detail</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/project-masjed-type-details', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
