...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/post-types', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function fetch_grid_data_draggable(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/post-types', array('action'=>'getorderlist'));?>",
			  data: objFormData,
			  dataType: "json",
			  success: function(data){
			  		hideShowLoader(false);
		...
		function saveorderData()  {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/post-types', array('action'=>'saveorder'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					mySmallAlert('Save Record', 'Post Types Order Saved successfully', 1);
					iActiveID = objMyPost.DATA.MY_ID;
		...
		fnBulkSave("<?php echo $this->url('adminpanel/post-types', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/post-types', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/post-types', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/post-types', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/post-types', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/post-types', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Types</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/post-types', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	...
	$(document).ready(function () {			
		populateOptionValues("post_category_id","<?php echo $this->url('adminpanel/post-categories', array('action'=>'getpostcategory'));?>","Select Post Categories");
