...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/project-categories', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function fetch_grid_data_draggable(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/project-categories', array('action'=>'getorderlist'));?>",
			  data: objFormData,
			  dataType: "json",
			  success: function(data){
			  		hideShowLoader(false);
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#name").val(), 'project_category', "name", "<?php echo $this->url('adminpanel/project-categories', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Project Category is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/project-categories', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'project category Entry Saved successfully', 1);
		...
		function saveorderData()  {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/project-categories', array('action'=>'saveorder'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					mySmallAlert('Save Record', 'Project Categories Order Saved successfully', 1);
					iActiveID = objMyPost.DATA.MY_ID;
		...
		fnBulkSave("<?php echo $this->url('adminpanel/project-categories', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/project-categories', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/project-categories', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/project-categories', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/project-categories', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/project-categories', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Project Category</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/project-categories', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
