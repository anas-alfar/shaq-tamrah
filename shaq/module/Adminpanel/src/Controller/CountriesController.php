...
class CountriesController extends AbstractActionController
{
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`iso_code_2`','`iso_code_3`','`default_currency`','`published`');
		if(!($this->memCached->hasItem('aula_country_data')) || !is_array($this->memCached->getItem('aula_country_data')))
		{	
			$sTable = 'view_country';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name FROM country_locale WHERE locale_id = '".$locale['id']."' AND country_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_country_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_country_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			foreach($rowset as $row)
			{
				...
				$csvData .= $row['id'].",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['iso_code_2']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['iso_code_3']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['default_currency']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['published']).",";
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name FROM country_locale WHERE locale_id = '".$locale['id']."' AND country_id = '".$row['id']."' ";
    ...
    {			
		...
		$projectTableLocale = new TableGateway('country_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" && $data[4] != "" && $data[5] != "" )
							{
								...
								$saveDataArray['iso_code_2']= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								$saveDataArray['iso_code_3']= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								$saveDataArray['default_currency'] 		= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("country_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['country_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('country_locale',$existRecordID,'country_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_country_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'country_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_country_data') && is_array($this->memCached->getItem('aula_country_data')))
			{
				$countries = $this->memCached->getItem('aula_country_data');
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("country_id=".$iMasterID));
				$this->memCached->setItem('aula_country_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'country';
		$tableNameLocale = 'country_locale';
        if ($this->request->isPost()) {
