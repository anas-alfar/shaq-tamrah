...
class GlAccountTypeController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`description`','`published`');
		if(!($this->memCached->hasItem('aula_glaccounttype_data')) || !is_array($this->memCached->getItem('aula_glaccounttype_data')))
		{	
			$sTable = 'view_gl_account_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM gl_account_type_locale WHERE locale_id = '".$locale['id']."' AND gl_account_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_glaccounttype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_glaccounttype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_gl_account_type WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM gl_account_type_locale WHERE locale_id = '".$locale['id']."' AND gl_account_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('gl_account_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('gl_account_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_gl_account_type',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
									if($existRecordID > 0)
									{
										continue;
									}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("gl_account_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['gl_account_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('gl_account_type_locale',$existRecordID,'gl_account_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_glaccounttype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=gl-account-types.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'gl_account_type_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_glaccounttype_data') && is_array($this->memCached->getItem('aula_glaccounttype_data')))
			{
				$gl_account_typees = $this->memCached->getItem('aula_glaccounttype_data');
				$rowset[0] = $gl_account_typees[$iID];
			}
			else
			{
				$projectTable = new TableGateway('gl_account_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("gl_account_type_id=".$iMasterID));
				$this->memCached->setItem('aula_glaccounttype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'gl_account_type';
		$tableNameLocale = 'gl_account_type_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['gl_account_type_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("gl_account_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getaccounttypeAction() 
	  {                
		$sql="select gl_account_type_id as id,name as name from gl_account_type_locale where locale_id = '".$this->global_locale_id."' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
