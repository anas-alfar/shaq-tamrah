...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/gl-account', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function fetch_grid_data_draggable(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/gl-account', array('action'=>'getorderlist'));?>",
			  data: objFormData,
			  dataType: "json",
			  success: function(data){
			  		hideShowLoader(false);
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#name_<?php echo $this->global_locale_id; ?>").val(), 'gl_account_locale', "name", "<?php echo $this->url('adminpanel/gl-account', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/gl-account', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Account Entry Saved successfully', 1);
		...
		function saveorderData()  {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/gl-account', array('action'=>'saveorder'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					mySmallAlert('Save Record', 'Account Order Saved successfully', 1);
					iActiveID = objMyPost.DATA.MY_ID;
		...
		$('#tabs').tabs();
			...
			fields : {
				...
				is_main : {
					validators : {
						notEmpty : {
							message : 'Please enter  is main'						
						},
						digits : {
							message : 'The is main is not valid'
						}
					}
				},
				is_leaf : {
					validators : {
						notEmpty : {
							message : 'Please enter  is leaf'						
						},
						digits : {
							message : 'The is leaf is not valid'
						}
					}
				},
				<?php
		...
		fnBulkSave("<?php echo $this->url('adminpanel/gl-account', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/gl-account', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/gl-account', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/gl-account', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/gl-account', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/gl-account', array('action'=>'delete'));?>"); 
					...
					loadScript("<?php echo $this->basePath(); ?>/public/admin/js/plugin/datatables/dataTables.bootstrap.min.js", function(){
						loadScript("<?php echo $this->basePath(); ?>/public/admin/js/plugin/datatable-responsive/datatables.responsive.min.js", function(){
							loadScript("<?php echo $this->basePath(); ?>/public/admin/js/plugin/bootstrapvalidator/bootstrapValidator.min.js", function(){
								loadScript("<?php echo $this->basePath(); ?>/public/admin/js/plugin/bootstraptree/bootstrap-tree.min.js", pagefunction)
							});	
						});	
					});
				});
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Account</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/gl-account', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script>
populateOptionValues("gl_account_type_id","<?php echo $this->url('adminpanel/gl-account-type', array('action'=>'getaccounttype'));?>","Select Account Types");
