...
class MediaTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`country_name`','`media_category_name`','`published`');
		if(!($this->memCached->hasItem('aula_mediatype_data')) || !is_array($this->memCached->getItem('aula_mediatype_data')))
		{	
			$sTable = 'view_media_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM media_type_locale WHERE locale_id = '".$locale['id']."' AND media_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_mediatype_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_mediatype_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_media_type WHERE 1 = 1";
			...
			$resultSet->initialize($resultData);        
			$rowset 			= $resultSet->toArray();
			$csvData .= "#ID,Country,Media Category,Published(Yes|No),";
			foreach($activeLocalesArray as $locale)
			{
			...
			foreach($rowset as $row)
			{
				...
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['media_category_name']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['published']).",";
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM media_type_locale WHERE locale_id = '".$locale['id']."' AND media_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('media_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('media_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" && $data[4] != "" )
							{
								...
								$getCategoryID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('media_category',$this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]),'name','id',$this->dbAdapter);
								
								
								$saveDataArray['country_id']= $getCountryID;
								$saveDataArray['media_category_id']= $getCategoryID;
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_media_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("media_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['media_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('media_type_locale',$existRecordID,'media_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_mediatype_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=media_types.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_mediatype_data') && is_array($this->memCached->getItem('aula_mediatype_data')))
			{
				$media_typees = $this->memCached->getItem('aula_mediatype_data');
				$rowset[0] = $media_typees[$iID];
			}
			else
			{
				$projectTable = new TableGateway('media_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("media_type_id=".$iMasterID));
				$this->memCached->setItem('aula_mediatype_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'media_type';
		$tableNameLocale = 'media_type_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				$masterData['media_category_id'] 		= $aData['media_category_id'];
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['media_type_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("media_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getmediatypeAction() 
	  {                
		$sql="select id as id,name as name,published from view_media_type where published='Yes' ";   
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
