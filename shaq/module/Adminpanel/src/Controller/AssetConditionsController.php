...
class AssetConditionsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_assetcondition_data')) || !is_array($this->memCached->getItem('aula_assetcondition_data')))
		{	
			$sTable = 'view_asset_condition';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM asset_condition_locale WHERE locale_id = '".$locale['id']."' AND asset_condition_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_assetcondition_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_assetcondition_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_asset_condition WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM asset_condition_locale WHERE locale_id = '".$locale['id']."' AND asset_condition_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('asset_condition',$this->dbAdapter);
		$projectTableLocale = new TableGateway('asset_condition_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("asset_condition_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_asset_condition',$detailData['name'],'name',$this->dbAdapter);	
									if($existRecordID > 0)
									{
										continue;
									}
									...
									$detailData['asset_condition_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('asset_condition_locale',$existRecordID,'asset_condition_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_assetcondition_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=asset-conditions.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'asset_condition_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_assetcondition_data') && is_array($this->memCached->getItem('aula_assetcondition_data')))
			{
				$asset_conditiones = $this->memCached->getItem('aula_assetcondition_data');
				$rowset[0] = $asset_conditiones[$iID];
			}
			else
			{
				$projectTable = new TableGateway('asset_condition', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("asset_condition_id=".$iMasterID));
				$this->memCached->setItem('aula_assetcondition_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'asset_condition';
		$tableNameLocale = 'asset_condition_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['asset_condition_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$projectTableLocale->update($detailData,array("asset_condition_id=".$iMasterID,"locale_id=".$locale['id']));
				}									
				$result['DBStatus'] = 'OK';
			}
			$this->memCached->setItem('aula_assetcondition_data','');
