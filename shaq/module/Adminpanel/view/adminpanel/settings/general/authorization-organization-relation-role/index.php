...
<div id="ribbon">
	...
	<ol class="breadcrumb">
		...
		<li>Organization Relation Role</li>
	</ol>
	<!-- end breadcrumb -->
...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#config_key").val(), 'configuration', "config_key", "<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Config key is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Organization Relation Role Entry Saved successfully', 1);
		...
		$('#frmForm').bootstrapValidator({
			...
			fields : {
				...
				admin_authorization_role_id: {
                    validators: {
                       callback: {
                            message: 'Please select admin authorization role',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
				...
				admin_authorization_organization_relation_id: {
                    validators: {
                       callback: {
                            message: 'Please select admin authorization organization relation',
                            callback: function (value, validator, $field) {
                               return (value != 0 && value != null && value != '');
                            }
                        }
		...
		fnBulkSave("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Organization Relation Role</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/authorization-organization-relation-role', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
populateOptionValues("admin_authorization_role_id","<?php echo $this->url('adminpanel/admin-authorization-role', array('action'=>'getrole'));?>","Select Admin Authorization Role");
