...
class ProjectTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`name`','`project_category`','`published`','`country_name`');
		if(!($this->memCached->hasItem('aula_projecttypes_data')) || !is_array($this->memCached->getItem('aula_projecttypes_data')))
		{	
			$sTable = 'view_project_type';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM project_type_locale WHERE locale_id = '".$locale['id']."' AND project_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_projecttypes_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_projecttypes_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_project_type WHERE 1 = 1";
			...
			$resultSet->initialize($resultData);        
			$rowset 			= $resultSet->toArray();
			$csvData .= "#ID,Project Categories,Published(Yes|No),Country,";
			foreach($activeLocalesArray as $locale)
			{
			...
			foreach($rowset as $row)
			{
				...
				$csvData .= $row['id'].",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['project_category']).",";
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM project_type_locale WHERE locale_id = '".$locale['id']."' AND project_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('project_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('project_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "" )
							{
								...
								$getProjectCategoryID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('project_category',$this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]),'name','id',$this->dbAdapter,$this->config['global_locale_id']);
								$saveDataArray['project_category_id']= $getProjectCategoryID;;
								...
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_project_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("project_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['project_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('project_type_locale',$existRecordID,'project_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_projecttypes_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=project_types.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_projecttypes_data') && is_array($this->memCached->getItem('aula_projecttypes_data')))
			{
				$project_types = $this->memCached->getItem('aula_projecttypes_data');
				$rowset[0] = $project_types[$iID];
			}
			else
			{
				$projectTable = new TableGateway('project_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("project_type_id=".$iMasterID));
				$this->memCached->setItem('aula_projecttypes_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'project_type';
		$tableNameLocale = 'project_type_locale';
        if ($this->request->isPost()) {
			...
			{
				$masterData = array();
				$masterData['project_category_id'] 		= $aData['project_category_id'];
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] 		= $locale['id'];
					$detailData['project_type_id'] 	= $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("project_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
