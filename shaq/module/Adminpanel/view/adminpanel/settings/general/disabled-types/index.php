...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/disabled-types', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_beneficiary_profile_disabled_type',"<?php echo $this->url('adminpanel/disabled-types', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Disabled Type for Selected Country Already exists !', 0);
				return false;
			}	
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Disabled Types Entry Saved successfully', 1);
		...
		$('#tabs').tabs();
			...
			oTable = $('#tblMasterList').DataTable({
				...
				"createdRow": function( nRow, data, dataIndex ) {
					...
					$( nRow ).find('td:eq(2)').attr('data-search', data[3]);
				},
		...
		fnBulkSave("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/disabled-types', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Disabled Types</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/disabled-types', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
