...
var gridData = [];
		function funCallBeforePopup(intID,name)
		{
			 $('#payment_modal_title').html(name);
		}
		function funCallAfterPopup(intID,name)
		{
			 var objFormData=
					 {
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/payment-method', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		var gridData1;
		function fetch_grid_data1(objFormData)
		{
			hideShowLoader(true);
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/payment-method-configuration', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
			  success: function(data){
			  		hideShowLoader(false);
					gridData1 = data.aaData;
					$("#tblpopup").find("tbody").html("");
					oTable1.clear().draw();
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_payment_method',"<?php echo $this->url('adminpanel/payment-method', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Payment Methods for Selected Country Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/payment-method', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Payment Method Entry Saved successfully', 1);
		...
		$('#tabs').tabs();
			...
			oTable = $('#tblMasterList').DataTable({
                ...
                "aoColumns": [
                    ...
                    {"bSearchable": false, "bSortable": false,
                        "mRender" : function (data, type, full) {
							return grid_buttons_popup(full[0],full[1]);
                        }
                    }
                ],
                "columnDefs": [
			...
			$('#tblMasterList tbody').on('click', 'td.details-control', function () {
				...
				else if(tr.hasClass('loaded'))
				{
					...
					$.ajax({
					  type: "POST",
					  url: "<?php echo $this->url('adminpanel/payment-method', array('action'=>'getgriddetailslist'));?>",
					  data:{'KEY_ID':KEY_ID},
					  dataType: "json",
					  success: function(data){
							hideShowLoader(false);
			...
			oTable1 = $('#tblpopup').DataTable({
				...
				"preDrawCallback" : function() {
					if (!responsiveHelper_tblMasterList1) {
						responsiveHelper_tblMasterList1 = new ResponsiveDatatablesHelper($('#tblpopup'), breakpointDefinition);
			...
			$("#tblpopup thead th input[type=text]").on( 'keyup change', function () {	    	
				oTable1
					.column( $(this).parent().index()+':visible' )
					.search( this.value )
					.draw();	            
			} );
			$("#tblpopup thead th select").on( 'change', function () {				
		...
		fnBulkSave("<?php echo $this->url('adminpanel/payment-method', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/payment-method', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/payment-method', array('action'=>'importcsv'));?>");
		fnPopup("<?php echo $this->url('adminpanel/payment-method', array('action'=>'popup'));?>");
		fnEdit("<?php echo $this->url('adminpanel/payment-method', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/payment-method', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/payment-method', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Payment Method</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/payment-method', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<div id="PopupGridModal" class="modal fade bs-example-modal-lg">
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              <h4 class="modal-title" id="ns_name_popup"><strong>Payment Method Configurations:<span id="payment_modal_title"></span></strong></h4>
         </div>
		 <div class="modal-footer">
		 		 <table id="tblpopup" class="table table-striped table-bordered" width="100%">	
					        <thead>
								<tr>
									<th></th>
									<th class="hasinput">
									...
									<th class="hasinput">
										<input type="text" class="form-control" placeholder="Filter Payment Method" />
									</th>
									<th class="hasinput">
					            ...
					            <tr>
									<th>Id</th>
				                    <th data-class="expand">Config Key</th>
				                    <th>Config Value</th>
				                    <th data-hide="phone">Locale</th>
