...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/family-flags', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			var isDuplicate = fn_validate_duplicate_multiple('view_beneficiary_profile_family_flag',"<?php echo $this->url('adminpanel/family-flags', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found. Family Flags for Selected Country Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/family-flags', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Family Flag Entry Saved successfully', 1);
				...
				<?php
					foreach($this->activeLocalesArray as $locale)
					{
						...
						flag_name_<?php echo $locale['id']?> : {
							validators : {
								notEmpty : {
									message : 'Please enter  flag name'						
								}
							}
						},
						<?php 
		...
		fnBulkSave("<?php echo $this->url('adminpanel/family-flags', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/family-flags', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/family-flags', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/family-flags', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/family-flags', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/family-flags', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Family Flag</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/family-flags', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
