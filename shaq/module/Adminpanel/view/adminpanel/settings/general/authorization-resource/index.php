...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			*/
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#name").val(), 'admin_authorization_resource', "name", "<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Admin Authorization Resource Entry Saved successfully', 1);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Admin Authorization Resource</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/authorization-resource', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
