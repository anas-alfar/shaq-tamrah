...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/branch-committee', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		$('#tabs').tabs();
			...
			$('#tblMasterList tbody').on('click', 'td.details-control', function () {
				...
				else if(tr.hasClass('loaded'))
				{
					...
					$.ajax({
					  type: "POST",
					  url: "<?php echo $this->url('adminpanel/branch-committee', array('action'=>'getgriddetailslist'));?>",
					  data:{'KEY_ID':KEY_ID},
					  dataType: "json",
					  success: function(data){
							hideShowLoader(false);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/branch-committee', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Branch Committee</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/branch-committee', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
