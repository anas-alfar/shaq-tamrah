...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/countries', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					...
					mySmallAlert('Save Record', 'Country Entry Saved successfully', 1);
		...
		$('#tabs').tabs();
			...
			oTable = $('#tblMasterList').DataTable({
				...
				"createdRow": function( nRow, data, dataIndex ) {
					...
					$( nRow ).find('td:eq(7)').attr('data-search', data[5]);
				},
			...
			fields : {
				iso_code_2 : {
					validators : {
						notEmpty : {
							message : 'Please enter iso code 2'						
						},
						callback: {
                            message: 'Maximum 2 charactors allowed ',
				...
				iso_code_3 : {
					validators : {
						notEmpty : {
							message : 'Please enter iso code 3'						
						},
						callback: {
                            message: 'Maximum 2 charactors allowed ',
                            callback: function (value, validator, $field) {
		...
		fnExport("<?php echo $this->url('adminpanel/countries', array('action'=>'exportcsv'));?>");
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Country</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/countries', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
