...
class PostTypesController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		{	
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM post_type_locale WHERE locale_id = '".$locale['id']."' AND post_type_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_posttypes_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_posttypes_data');
		$iTotal = count($rowset);
	...
	{	
		$projectTable = new TableGateway('post_type',$this->dbAdapter);	 
        if ($this->request->isPost()) {		
			...
			$this->memCached->setItem('aula_posttypes_data','');
			$result['DBStatus'] = 'OK';
		}
		else
        {
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_post_type WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM post_type_locale WHERE locale_id = '".$locale['id']."' AND post_type_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('post_type',$this->dbAdapter);
		$projectTableLocale = new TableGateway('post_type_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
						...
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != ""&& $data[4] != "" && $data[5] != ""&& $data[6] != "" )
							{
								...
								$fnameValPair['post_category_id']=$saveDataArray['post_category_id'];
								$fnameValPair['country_id ']=$saveDataArray['country_id'];
								
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_post_type',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
								}	
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("post_type_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['post_type_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('post_type_locale',$existRecordID,'post_type_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=post-types.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_posttypes_data') && is_array($this->memCached->getItem('aula_posttypes_data')))
			{
				$post_type = $this->memCached->getItem('aula_posttypes_data');
				$rowset[0] = $post_type[$iID];
			}
			else
			{
				$projectTable = new TableGateway('post_type', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("post_type_id=".$iMasterID));
				$this->memCached->setItem('aula_posttypes_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'post_type';
		$tableNameLocale = 'post_type_locale';
        if ($this->request->isPost()) {
			...
			{
				$masterData = array();
				$sql="select count(id) sequence  from post_type order by sequence DESC LIMIT 1";
				...
				$masterData['post_category_id'] = $aData['post_category_id'];
				$masterData['country_id'] 		= $aData['country_id'];
				$masterData['parent_id'] 		= 0;
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['locale_id'] = $locale['id'];
					$detailData['post_type_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['name'] = $aData['name_'.$locale['id']];
					$detailData['description'] = $aData['description_'.$locale['id']];
					$rowset = $projectTableLocale->select(array("post_type_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
