...
class DeathReasonsController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		if(!($this->memCached->hasItem('aula_deathreason_data')) || !is_array($this->memCached->getItem('aula_deathreason_data')))
		{	
			$sTable = 'view_death_reason';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description FROM death_reason_locale WHERE locale_id = '".$locale['id']."' AND death_reason_id = '".$aRow['id']."' ";
			...
			$this->memCached->setItem('aula_deathreason_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_deathreason_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_death_reason WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description FROM death_reason_locale WHERE locale_id = '".$locale['id']."' AND death_reason_id = '".$row['id']."' ";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('death_reason',$this->dbAdapter);
		$projectTableLocale = new TableGateway('death_reason_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("death_reason_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['death_reason_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('death_reason_locale',$existRecordID,'death_reason_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_deathreason_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=death_reasons.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_deathreason_data') && is_array($this->memCached->getItem('aula_deathreason_data')))
			{
				$death_reason = $this->memCached->getItem('aula_deathreason_data');
				$rowset[0] = $death_reason[$iID];
			}
			else
			{
				$projectTable = new TableGateway('death_reason', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("death_reason_id=".$iMasterID));
				$this->memCached->setItem('aula_deathreason_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'death_reason';
		$tableNameLocale = 'death_reason_locale';
        if ($this->request->isPost()) {
