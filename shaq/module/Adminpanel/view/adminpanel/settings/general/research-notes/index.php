...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/research-notes', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			var isDuplicate = fn_validate_duplicate($("#researcher_name_<?php echo $this->global_locale_id; ?>").val(), 'beneficiary_profile_research_notes_locale', "researcher_name", "<?php echo $this->url('adminpanel/research-notes', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
				mySmallAlert('Duplicate Error...!', 'Duplicate Found.Researcher Name is Already exists !', 0);
				return false;
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/research-notes', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Research Notes Entry Saved successfully', 1);
		...
		fnBulkSave("<?php echo $this->url('adminpanel/research-notes', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/research-notes', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/research-notes', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/research-notes', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/research-notes', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/research-notes', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Marital status</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/research-notes', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
