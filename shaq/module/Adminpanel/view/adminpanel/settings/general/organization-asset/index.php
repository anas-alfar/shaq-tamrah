...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/organization-asset', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			if (status == "Active")
			{
				var isDuplicate = fn_validate_duplicate_multiple('organization_asset',"<?php echo $this->url('adminpanel/organization-asset', array('action'=>'validateduplicate'));?>",iActiveID,objFormData);
				if (isDuplicate) {
					mySmallAlert('Duplicate Error...!', 'Duplicate Found. Asset Id is Already exists !', 0);
					return false;
				}
			}
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Organization Asset Entry Saved successfully', 1);
                ...
                "aoColumns": [
                    ...
                    { "bSearchable": true, "bSortable": true,
                        "mRender" : function (data, type, full) {							
							return grid_switch_active(full[0],'status',full[10],'Active');							
						}
					},
		...
		fnBulkSave("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'bulksave'));?>");
		fnImport("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'importcsv'));?>");
		fnExport("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'exportcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/organization-asset', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Organization Asset</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/organization-asset', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
...
<script language="javascript">
	$(document).ready(function () {			
		...
		populateOptionValues("asset_unit_id","<?php echo $this->url('adminpanel/asset-unit-types', array('action'=>'getassetunit'));?>","Select Asset Unit Type");
	});
