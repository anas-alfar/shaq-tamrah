...
<div id="ribbon">
	...
	<ol class="breadcrumb">
		<li>Beneficiary</li>
		<li>Beneficiary Profile Asset Received</li>
	</ol>
	<!-- end breadcrumb -->
...
<script type="text/javascript">
		...
		function fetch_grid_data(objFormData)
		{
			...
			$.ajax({
			  type: "POST",
			  url: "<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'list'));?>",
			  data: objFormData,
			  dataType: "json",
		...
		function savefrmFormData()  {
			...
			//Validate  duplicate
			<?php /*?>var isDuplicate = fn_validate_duplicate($("#name").val(), 'beneficiary_beneficiary-profile-asset-received', "name", "<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'validateduplicate'));?>",iActiveID);
			if (isDuplicate) {
			...
			hideShowLoader(true);
			var objMyPost = AJAX_Post("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'save'));?>", objFormData);
			if (objMyPost.ERR_NO === 0) {
				if (objMyPost.DATA.DBStatus === 'OK') {
					strActionMode = "EDIT";
					hideShowLoader(false);							
					fullscreenModeChange('btnSave');
					mySmallAlert('Save Record', 'Beneficiary Profile Asset Receiveds Entry Saved successfully', 1);
		...
		$('#tabs').tabs();
			...
			oTable = $('#tblMasterList').DataTable({
				...
				"oLanguage": {
					"sSearch": '<span class="input-beneficiary-profile-asset-received-addon"><i class="glyphicon glyphicon-search"></i></span>'
				},
				"bProcessing": false,
                "bServerSide": false,
		...
		fnBulkSave("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'bulksave'));?>");
		fnExport("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'exportcsv'));?>");
		fnImport("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'importcsv'));?>");
		fnEdit("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'getrec'));?>");
		fnView("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'getrec'));?>");
		fnDelete("<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'delete'));?>"); 
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
              ...
              <h4 class="modal-title" id="ns_name_popup"><strong>Import Beneficiary Profile Asset Received</strong></h4>
         </div>
		 <form action="" class="smart-form" enctype="multipart/form-data" name="importcsvform" id="importcsvform">
			<fieldset>
				<section>
					...
					<div class="note note-error">Please <a href="<?php echo $this->url('adminpanel/beneficiary-profile-asset-received', array('action'=>'downloadtemplate'));?>" target="_blank">download template </a>first (if you don't have download yet!)
					</div>
				</section>
			</fieldset>			
		 </form>
