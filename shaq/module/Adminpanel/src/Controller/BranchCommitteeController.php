...
class BranchCommitteeController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`id`','`name`','`agenda`','`annual_plan`','`organization_branch_name`','`published`');
		if(!($this->memCached->hasItem('aula_branchcommittee_data')) || !is_array($this->memCached->getItem('aula_branchcommittee_data')))
		{	
			$sTable = 'view_organization_branch_committee';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT name,description,agenda,annual_plan FROM organization_branch_committee_locale WHERE locale_id = '".$locale['id']."' AND organization_branch_committee_id = '".$aRow['id']."' ";
					...
					$aRow['agenda_'.$locale['id']] = $rowset_locale[0]['agenda'];
					$aRow['annual_plan_'.$locale['id']] = $rowset_locale[0]['annual_plan'];
				}
				$rowsetCache[$aRow['id']] = $aRow;
			}
			$this->memCached->setItem('aula_branchcommittee_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_branchcommittee_data');
		$iTotal = count($rowset);
	...
	public function getgriddetailslistAction()
	{
		...
		$committee = $this->AdminfunctionsPlugin()->getSingleRecord($iID,$this->memCached,'aula_branchcommittee_data','view_organization_branch_committee',$this->dbAdapter);
		
		$committee_user = $this->AdminfunctionsPlugin()->getSingleRecord($committee['id'],$this->memCached,'aula_branchcommittee_data','view_organization_branch_committee',$this->dbAdapter);	
		$user_position = $this->AdminfunctionsPlugin()->getSingleRecord2('id',$iID,'view_organization_user_position',$this->dbAdapter);
		
		$grid_list = '';
		$grid_list .= '<h5 class="gridDetailSectionHeading">Committee User & Position:</h5>';
		$grid_list .= '<table cellpadding="5" cellspacing="0" border="0" class="table table-hover table-condensed gridDetailTable" id="branch_committee_detail_tbl">';
		$grid_list .= '<tr >
							<td>User : </td>
							<td>'.$committee_user['organization_user'].'</td>
						</tr>';
		$grid_list .= '<tr >
							<td>Position : </td>
							<td>'.$user_position['title'].'</td>
						</tr>';
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_organization_branch_committee WHERE 1 = 1";
			...
			$resultSet->initialize($resultData);        
			$rowset 			= $resultSet->toArray();
			$csvData .= "#ID,Published(Yes|No),Organization User,Organization Branch,";
			foreach($activeLocalesArray as $locale)
			{
				$csvData .= "Name(".$locale['name']."),";
				$csvData .= "Description(".$locale['name']."),";
				$csvData .= "Agenda(".$locale['name']."),";
				$csvData .= "Annual Plan(".$locale['name']."),";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT name,description,agenda,annual_plan FROM organization_branch_committee_locale WHERE locale_id = '".$locale['id']."' AND organization_branch_committee_id = '".$row['id']."' ";
						...
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['agenda']).",";
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['annual_plan']).",";
					}
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('organization_branch_committee',$this->dbAdapter);
		$projectTableLocale = new TableGateway('organization_branch_committee_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								$detailData['agenda'] = $data[$column_index++];
								$detailData['annual_plan'] = $data[$column_index++];
								$detailData['owner_organization_id'] = self::$Aula_OwnerOrgID;
								
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_organization_branch_committee',$detailData['name'],'name',$this->dbAdapter,$data[0]);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("organization_branch_committee_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['organization_branch_committee_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('organization_branch_committee_locale',$existRecordID,'organization_branch_committee_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_branchcommittee_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		$csvData .= "#ID,Published(Yes|No),Organization,Organization User,Organization Branch,";
		foreach($activeLocalesArray as $locale)
		{
			$csvData .= "Name(".$locale['name']."),";
			$csvData .= "Description(".$locale['name']."),";
		...
		header("Content-Disposition: attachment; filename=branch-committee.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'organization_branch_committee_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_branchcommittee_data') && is_array($this->memCached->getItem('aula_branchcommittee_data')))
			{
				$organization_branch_committee = $this->memCached->getItem('aula_branchcommittee_data');
				$rowset[0] = $organization_branch_committee[$iID];
			}
			else
			{
				$projectTable = new TableGateway('organization_branch_committee', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("organization_branch_committee_id=".$iMasterID));
				$this->memCached->setItem('aula_branchcommittee_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'organization_branch_committee';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'organization_branch_committee';
		$tableNameLocale = 'organization_branch_committee_locale';
        if ($this->request->isPost()) {
			...
			if ($this->request->getPost("pAction") == "ADD")
			{
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$detailData['agenda'] 			 = $aData['agenda_'.$locale['id']];
					$detailData['annual_plan']		 = $aData['annual_plan_'.$locale['id']];
					
					
					$detailData['locale_id'] 		 				= $locale['id'];
					$detailData['organization_branch_committee_id'] = $iMasterID;
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("organization_branch_committee_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
		...
		public function getbranchcommitteeAction() 
	  {                
		$sql="select organization_branch_committee_id as id,name as name from organization_branch_committee_locale where locale_id = '".$this->global_locale_id."' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
