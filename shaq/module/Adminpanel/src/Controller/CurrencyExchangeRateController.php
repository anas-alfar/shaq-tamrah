...
class CurrencyExchangeRateController extends AbstractActionController
{
	private $dbAdapter;
	...
	public function fnGrid()
	{
		...
		$aColumns = array( '`id`','`f_currency`','`t_currency`','`exchange_rate`','`status`');
		if(!($this->memCached->hasItem('aula_currencyexchangerate_data')) || !is_array($this->memCached->getItem('aula_currencyexchangerate_data')))
		{	
			$sTable = 'view_currency_exchange_rate';
			...
			$this->memCached->setItem('aula_currencyexchangerate_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_currencyexchangerate_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';
			$sql = "SELECT * FROM view_currency_exchange_rate WHERE 1 = 1";	
			...
			$csvData .= "#ID,From Currency,To Currency,Exchange Rate,Status(Active|Inactive)";
			$csvData .= "\n";
			foreach($rowset as $row)
			{
				$csvData .= $row['id'].",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['from_currency']).",";
				$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($row['to_currency']).",";
    ...
    {			
        $projectTable = new TableGateway('currency_exchange_rate',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" && $data[4] != "" )
							{
								...
								$saveDataArray = array();
								$column_index = 1;
								$saveDataArray['from_currency'] 		= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								$saveDataArray['to_currency'] 			= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								$saveDataArray['exchange_rate']			= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								$saveDataArray['status'] 				= $this->AdminfunctionsPlugin()->importDataValidate($data[$column_index++]);
								
								$fnameValPair = array();
								$fnameValPair['from_currency ']=$saveDataArray['from_currency'];
								$fnameValPair['to_currency ']=$saveDataArray['to_currency'];
								
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicatemultipleCSV('view_currency_exchange_rate',$data[0],$fnameValPair,$this->dbAdapter);	
								if($existRecordID > 0)
								{
									continue;
						...
						$this->memCached->setItem('aula_currencyexchangerate_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=currency-exchange-rate.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_currencyexchangerate_data') && is_array($this->memCached->getItem('aula_currencyexchangerate_data')))
			{
				$currency_exchange_rate = $this->memCached->getItem('aula_currencyexchangerate_data');
				$rowset[0] = $currency_exchange_rate[$iID];
			}
			else
			{
				$projectTable = new TableGateway('currency_exchange_rate', $this->dbAdapter);
	...
	public function getexchangerateAction() 
	  {                
		$sql="select id as id,exchange_rate as name from currency_exchange_rate where 1=1 ";	
		
		if ($this->request->getPost("from_currency") !='' &&$this->request->getPost("from_currency") >= 0) 
		$sql .= " AND from_currency = '".$this->request->getPost("from_currency")."' ";	        
