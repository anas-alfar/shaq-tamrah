...
class FamilyFlagsController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`flag_name`','`country_name`','`published`');
		if(!($this->memCached->hasItem('aula_familyflag_data')) || !is_array($this->memCached->getItem('aula_familyflag_data')))
		{	
			$sTable = 'view_beneficiary_profile_family_flag';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT flag_name FROM beneficiary_profile_family_flag_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_family_flag_id = '".$aRow['id']."' ";
					...
					$aRow['flag_name_'.$locale['id']] = $rowset_locale[0]['flag_name'];
				}
				$rowsetCache[$aRow['id']] = $aRow;
			}
			$this->memCached->setItem('aula_familyflag_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_familyflag_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_beneficiary_profile_family_flag WHERE 1 = 1";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT flag_name FROM beneficiary_profile_family_flag_locale WHERE locale_id = '".$locale['id']."' AND beneficiary_profile_family_flag_id = '".$row['id']."' ";
						$statement_locale	= $this->dbAdapter->createStatement($sQuery_locale1, $optionalParameters);        
						$resultData_locale	= $statement_locale->execute();        
						$resultSet_locale	= new ResultSet; 			   
						$resultSet_locale->initialize($resultData_locale);        
						$rowset_locale		= $resultSet_locale->toArray();
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['flag_name']).",";
					}
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('beneficiary_profile_family_flag',$this->dbAdapter);
		$projectTableLocale = new TableGateway('beneficiary_profile_family_flag_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
					   	...
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" )
							{
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("beneficiary_profile_family_flag_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
									...
									$detailData['beneficiary_profile_family_flag_id'] = $existRecordID;
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('beneficiary_profile_family_flag_locale',$existRecordID,'beneficiary_profile_family_flag_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
						...
						$this->memCached->setItem('aula_familyflag_data','');
					}
					else
					{
						$result['status'] = 'ERR';
	...
	public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=family-flags.csv");
		echo $csvData;
		exit;
	}
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_familyflag_data') && is_array($this->memCached->getItem('aula_familyflag_data')))
			{
				$beneficiary_profile_family_flages = $this->memCached->getItem('aula_familyflag_data');
				$rowset[0] = $beneficiary_profile_family_flages[$iID];
			}
			else
			{
				$projectTable = new TableGateway('beneficiary_profile_family_flag', $this->dbAdapter);
	...
	public function  deleteAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("beneficiary_profile_family_flag_id=".$iMasterID));
				$this->memCached->setItem('aula_familyflag_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    {        
		$tableName = 'beneficiary_profile_family_flag';
        if ($this->request->isPost()) {
    ...
    { 
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);       
		$tableName = 'beneficiary_profile_family_flag';
		$tableNameLocale = 'beneficiary_profile_family_flag_locale';
        if ($this->request->isPost()) {
