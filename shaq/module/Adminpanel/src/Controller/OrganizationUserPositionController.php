...
class OrganizationUserPositionController extends AbstractActionController
{
	private $dbAdapter;
	...
	{
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
		$aColumns = array( '`id`','`title`','`description`','`qualifications`','`responsibilities`');
		if(!($this->memCached->hasItem('aula_organizationuserposition_data')) || !is_array($this->memCached->getItem('aula_organizationuserposition_data')))
		{	
			$sTable = 'view_organization_user_position';
			...
			foreach($rowset as $aRow)
			{
				...
				{
					$sQuery_locale 		= "SELECT title,description,qualifications,responsibilities FROM organization_user_position_locale WHERE locale_id = '".$locale['id']."' AND organization_user_position_id = '".$aRow['id']."' ";
					...
					$aRow['title_'.$locale['id']] = $rowset_locale[0]['title'];
					$aRow['description_'.$locale['id']] = $rowset_locale[0]['description'];
					$aRow['qualifications_'.$locale['id']] = $rowset_locale[0]['qualifications'];
					$aRow['responsibilities_'.$locale['id']] = $rowset_locale[0]['responsibilities'];
					
				}
				$rowsetCache[$aRow['id']] = $aRow;
			}
			$this->memCached->setItem('aula_organizationuserposition_data',$rowsetCache);
		}
			
		$rowset = $this->memCached->getItem('aula_organizationuserposition_data');
		$iTotal = count($rowset);
	...
	public function exportcsvAction()
	{
		if ($this->request->isPost()) 
		{
			...
			$csvData = '';		
			$sql = "SELECT * FROM view_organization_user_position WHERE 1 = 1";
			...
			foreach($activeLocalesArray as $locale)
			{
				$csvData .= "Title(".$locale['name']."),";
				$csvData .= "Description(".$locale['name']."),";
				$csvData .= "Qualifications(".$locale['name']."),";
				$csvData .= "Responsibilities(".$locale['name']."),";
			...
			foreach($rowset as $row)
			{
				...
				foreach($activeLocalesArray as $locale)
					{
						$sQuery_locale1 		= "SELECT title,description,qualifications,responsibilities FROM organization_user_position_locale WHERE locale_id = '".$locale['id']."' AND organization_user_position_id = '".$row['id']."' ";
						...
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['title']).",";
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['description']).",";
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['qualifications']).",";
						$csvData .= $this->AdminfunctionsPlugin()->exportDataValidate($rowset_locale[0]['responsibilities']).",";
    ...
    {			
		$activeLocalesArray = $this->AdminfunctionsPlugin()->getActiveLocales($this->dbAdapter);
        $projectTable = new TableGateway('organization_user_position',$this->dbAdapter);
		$projectTableLocale = new TableGateway('organization_user_position_locale',$this->dbAdapter);
		if ($this->request->isPost()) {
			...
			{            
				...
				{	
					if (($handle = fopen($importFilePath, "r")) !== FALSE) 
					{
						fgetcsv($handle);   
					   	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
							if($data[1] != "" && $data[2] != "" && $data[3] != "" && $data[4] != "")
							{
								...
								$column_index = 1;
								$detailData = array();
								$detailData['title'] 					= $data[$column_index++];
								$detailData['description']			 	= $data[$column_index++];
								$detailData['qualifications'] 			= $data[$column_index++];
								$detailData['responsibilities'] 		= $data[$column_index++];
								
								$existRecordID = $this->AdminfunctionsPlugin()->validateduplicateCSV('view_organization_user_position',$detailData['title'],'title',$this->dbAdapter,$data[0]);	
								if($existRecordID > 0)
								{
									continue;
								}
								...
								if($existRecordID > 0)
								{
									...
									$projectTableLocale->update($detailData,array("organization_user_position_id=".$existRecordID,"locale_id=".$this->config['global_locale_id']));	
								}
								else
								{
								...
								foreach($activeLocalesArray as $locale)
								{
									...
									$existLocaleRecordID = $this->AdminfunctionsPlugin()->validateduplicateLocaleCSV('organization_user_position_locale',$existRecordID,'organization_user_position_id','id',$this->dbAdapter,$locale['id'],'locale_id'); 
									$detailData = array();
									$detailData['title'] 				= $data[$column_index++];
									$detailData['description']		 	= $data[$column_index++];
									$detailData['qualifications'] 		= $data[$column_index++];
						...
						$this->memCached->setItem('aula_organizationuserposition_data','');
					}
					else
					{
						$result['status'] = 'ERR';
...
public function downloadtemplateAction()
	{
		...
		header("Content-Disposition: attachment; filename=organization-user-position.csv");
		echo $csvData;
		exit;
	}
	public function validateduplicateAction()
    {
        if ($this->request->isPost()) {
			...
			$this->AdminfunctionsPlugin()->validateduplicatelocale($tableName,$ID,$fieldName,$EDIT_ID,'organization_user_position_id',$this->dbAdapter,$this->config);           
        }
		else {
	...
	public function getrecAction()
    {
        ...
        if ($this->request->isPost()) {
			...
			if($this->memCached->hasItem('aula_organizationuserposition_data') && is_array($this->memCached->getItem('aula_organizationuserposition_data')))
			{
				$organization_user_position = $this->memCached->getItem('aula_organizationuserposition_data');
				$rowset[0] = $organization_user_position[$iID];
			}
			else
			{
				$projectTable = new TableGateway('organization_user_position', $this->dbAdapter);
   ...
   if ($this->request->isPost()) {
            ...
            if ($this->request->getPost("pAction") == "DELETE") {
				...
				$projectTableLocale->delete(array("organization_user_position_id=".$iMasterID));
				$this->memCached->setItem('aula_organizationuserposition_data','');
                $result['DBStatus'] = 'OK';
                $result = json_encode($result);
                echo $result;
    ...
    { 
		...
		$tableNameLocale = 'organization_user_position_locale';
        if ($this->request->isPost()) {
		...
		if ($this->request->getPost("pAction") == "ADD")
			{
				$masterData = array();
				$masterData['organization_id'] 					= self::$Aula_UID;
				...
				{
					$detailData = array();
					$detailData['title'] 							= $aData['title_'.$locale['id']];
					$detailData['description'] 						= $aData['description_'.$locale['id']];
					$detailData['qualifications'] 					=$aData['qualifications_'.$locale['id']];
					$detailData['responsibilities'] 				= $aData['responsibilities_'.$locale['id']];
					$detailData['locale_id'] 						= $locale['id'];
					$detailData['organization_user_position_id'] 	= $iMasterID;
					$detailData['organization_id']				 	= self::$Aula_UID;
					$detailData['owner_organization_id'] 			= self::$Aula_OwnerOrgID;
					$detailData['owner_organization_user_id']		= self::$Aula_OwnerOrgUserID;
					$projectTableLocale->insert($detailData);	
			...
			{			
				...
				foreach($activeLocalesArray as $locale)
				{
					...
					$rowset = $projectTableLocale->select(array("organization_user_position_id=".$iMasterID,"locale_id=".$locale['id']));
					$rowset = $rowset->toArray();
	...
	public function getuserpositionAction() 
	  {                
		$sql="select organization_user_position_id as id,title as name from organization_user_position_locale where locale_id = '".$this->global_locale_id."' ";		        
		$optionalParameters=array();        
		$statement 		   = $this->dbAdapter->createStatement($sql, $optionalParameters);       
